...
public class Actionbar {
    ...
    public static final int ID_SETTING_OPEN_TAB = 682;

    private static final int ID_WIDGET_ACTION_BAR = 640;
    public static final int ID_WIDGET_ABILITY_BOOK = 275;
    ...
    public static boolean open() {
        if (!isOpen()) {
            ...
            if (button.validate() && button.click(true)) {
                final Timer timer = new Timer(Random.nextInt(1500, 2000));
    ...
    public static boolean contains(final Ability ability) {
        return getSlotWithAbility(ability) != null;
    }
    ...
    public static boolean isBerserkActive() {
        return Settings.get(692) > 0;
    }
    ...
    public enum Slot {
        ...
        public boolean activate(boolean sendKey) {
            ...
            if (sendKey) {
                Keyboard.sendKey(Widgets.get(ID_WIDGET_ACTION_BAR, widgetChildText).getText().toLowerCase().charAt(0));
                return true;
            } else {
    ...
    public enum GeneralTab {
        ...
        public static final int MASK_TAB = 0x7;

        private int[] masks;
        private int widgetChildId;

        GeneralTab(final int widgetChildId, final int... masks) {
        ...
        public boolean isOpen() {
            ...
            for (final int mask : masks)
                if ((Settings.get(ID_SETTING_OPEN_TAB) & MASK_TAB) == mask)
                    return true;
            return false;
        }
        ...
        public boolean open() {
            ...
            if (Tabs.ABILITY_BOOK.open()) {
                final WidgetChild widgetChild = Widgets.get(ID_WIDGET_ABILITY_BOOK, widgetChildId);
                if (widgetChild.validate() && widgetChild.click(true)) {
                ...
                if (isOpen()) {
                    final WidgetChild innerTab = Widgets.get(ID_WIDGET_ABILITY_BOOK, 42);
                    if (innerTab.visible())
                        innerTab.click(true);
                }
            }
        ...
        public static GeneralTab getOpenTab() {
            for (final GeneralTab generalTab : GeneralTab.values())
                if (generalTab.isOpen())
                    return generalTab;
            return null;
        }
    }
    ...
    public enum InnerTab {
        ATTACK_ABILITIES(GeneralTab.MELEE, 1, 0, 42),
        STRENGTH_ABILITIES(GeneralTab.MELEE, 2, 0, 45),
        RANGED_ABILITIES(GeneralTab.RANGED, 3, 0, 42),

        MAGIC_ABILITIES(GeneralTab.MAGIC, 0, 0x0, 42),
        MAGIC_COMBAT_SPELLS(GeneralTab.MAGIC, 0, 0x40000, 45),
        MAGIC_TELEPORT_SPELLS(GeneralTab.MAGIC, 0, 0x80000, 47),
        MAGIC_SKILLING_SPELLS(GeneralTab.MAGIC, 0, 0xc0000, 49),

        DEFENCE_ABILITIES(GeneralTab.OTHER, 5, 0, 42),
        CONSTITUTION_ABILITIES(GeneralTab.OTHER, 6, 0, 45);

        public static final int MASK_TAB = 0x7;
        public static final int MAGIC_MASK = 0xc0000;

        private GeneralTab generalTab;
        private int regularMask;
        private int magicMask;
        private int widgetChildId;

        InnerTab(final GeneralTab generalTab, final int regularMask, final int magicMask, final int widgetChildId) {
            this.generalTab = generalTab;
            this.regularMask = regularMask;
            this.magicMask = magicMask;
            this.widgetChildId = widgetChildId;
        }
        ...
        public boolean isOpen() {
            if (!generalTab.isOpen())
                return false;
            if (regularMask > 0) // Other tabs
                return (Settings.get(ID_SETTING_OPEN_TAB) & MASK_TAB) == regularMask;
            else // Magic tab
                return (Settings.get(ID_SETTING_OPEN_TAB) & MAGIC_MASK) == magicMask;
        }
        ...
        public static InnerTab getOpenTab() {
            for (final InnerTab innerTab : InnerTab.values())
                if (innerTab.isOpen())
                    return innerTab;
            return null;
        }
